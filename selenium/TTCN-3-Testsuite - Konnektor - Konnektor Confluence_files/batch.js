...
Confluence.Templates.Sidebar.renderLinksSection = function(opt_data, opt_ignored) {
  ...
  if (opt_data.links.length) {
    ...
    for (var linkIndex130 = 0; linkIndex130 < linkListLen130; linkIndex130++) {
      var linkData130 = linkList130[linkIndex130];
      output += '<li class="acs-nav-item ' + soy.$$escapeHtml(linkData130.styleClass) + ((opt_data.collectorToHighlight && linkData130.collectorKey == opt_data.collectorToHighlight) ? ' current-item' : '') + '"' + ((linkData130.collectorKey) ? 'data-collector-key="' + soy.$$escapeHtml(linkData130.collectorKey) + '"' : '') + '><a class="acs-nav-item-link tipsy-enabled" href="' + soy.$$escapeHtml(linkData130.url) + '" data-collapsed-tooltip="' + soy.$$escapeHtml(linkData130.tooltip) + '">' + ((linkData130.styleClass == 'pinned_attachment') ? '<span class="aui-icon aui-icon-small aui-iconfont-attachment"></span>' : '<span class="icon"></span>') + '<span class="acs-nav-item-label">' + soy.$$escapeHtml(linkData130.title) + '</span></a></li>';
    }
    output += '</ul></div>';
  }
  return output;
...
Confluence.Templates.Sidebar.pagetreeList = function(opt_data, opt_ignored) {
  ...
  for (var itemIndex212 = 0; itemIndex212 < itemListLen212; itemIndex212++) {
    ...
    output += Confluence.Templates.Sidebar.pagetreeItem(itemData212);
  }
  output += '</ul>';
  return output;
};
...
Confluence.Templates.Sidebar.renderSpaceToolsSection = function(opt_data, opt_ignored) {
  ...
  for (var linkIndex282 = 0; linkIndex282 < linkListLen282; linkIndex282++) {
    ...
    output += '<div data-label="' + soy.$$escapeHtml(linkData282.title) + '" data-class="' + soy.$$escapeHtml(linkData282.styleClass) + '" data-href="' + soy.$$escapeHtml(linkData282.url) + '">' + soy.$$escapeHtml(linkData282.title) + '</div>';
  }
...
Confluence.Templates.Sidebar.spaceToolsMenu = function(opt_data, opt_ignored) {
  ...
  if (opt_data.isAuiFiveSeven) {
    ...
    var linkList333 = opt_data.spaceToolLinks;
    ...
    for (var linkIndex333 = 0; linkIndex333 < linkListLen333; linkIndex333++) {
      ...
      output += '<li><a href="' + soy.$$escapeHtml(linkData333.href) + '" title="' + soy.$$escapeHtml(linkData333.label) + '">' + soy.$$escapeHtml(linkData333.label) + '</a></li>';
    }
    output += '</ul></div>';
    if (opt_data.spaceLinks.length > 0) {
      output += '<div class="aui-dropdown2-section"><ul class="space-operations">';
      var linkList346 = opt_data.spaceLinks;
      var linkListLen346 = linkList346.length;
      for (var linkIndex346 = 0; linkIndex346 < linkListLen346; linkIndex346++) {
        var linkData346 = linkList346[linkIndex346];
        output += '<li><a class="' + soy.$$escapeHtml(linkData346.className) + '" href="' + soy.$$escapeHtml(linkData346.href) + '" title="' + soy.$$escapeHtml(linkData346.label) + '">' + soy.$$escapeHtml(linkData346.label) + '</a></li>';
      }
      output += '</ul></div>';
    }
    output += '</div>';
...
define("confluence-like/like",["ajs","confluence/legacy","jquery","confluence/meta","confluence/flag"],function(d,h,e,g){function n(a){return d.contextPath()+"/rest/likes/1.0/content/"+a+"/likes"}function q(){return 0<(g.get("remote-user")||"").length&&!1!==g.get("remote-user-has-licensed-access")}function o(a,b,c){var e=g.get("remote-user")||"",b=h.Likes.LikeSummaryFactory.getLikeSummary(a.likes,b,e);if(!b.key&&i&&("page"===a.content_type||"blogpost"===a.content_type))b.text="Be the first to like this";
a=[];a.push(b.text);a=a.concat(b.args);(a=d.format.apply(d,a))&&0<a.length?(i&&c.addClass("like-summary-margin-left"),c.html(a)):(c.removeClass("like-summary-margin-left"),c.empty());a&&(c.find(".likes").click(m.showLikeUsers),h.Binder.userHover())}function l(a,b,c){var r=g.get("remote-user")||"";if(void 0===a)throw Error("type is required");if(void 0===b)throw Error("contentId is required");if(void 0===c)throw Error("contentType is required");return function(){if("object"!==typeof this||!this.nodeType||
1!==this.nodeType||"A"!==this.nodeName)throw Error("this handler should be bound to a DOM anchor element");var f=e(this),h=arguments.callee,i=f.next(".like-summary");f.addClass("disabled");e.ajax({type:a===j?"POST":"DELETE",url:n(b),contentType:"application/json",data:{"atlassian-token":g.get("atlassian-token")},dataType:"json",timeout:5E3}).fail(function(){var b;b=a===j?"Like failed":"Unlike failed";var c=i.siblings(".like-error");0===c.length?i.after('<span class="like-error" title="'+
b+'"></span>'):c.attr("title",b)}).success(function(){i.attr("data-liked",a===j);i.parent().find(".like-error").remove();f.unbind("click",h).bind("click",a===j?l(p,b,c):l(j,b,c)).find(".like-button-text").html(a===j?"Unlike":"Like");k[b]=k[b]||{content_type:c,likes:[]};a===j?k[b].likes.push({user:{name:r}}):k[b].likes=e.grep(k[b].likes,function(a){return a.user.name!==r});a===j&&d.trigger("analytics",{name:"confluence."+c+".like.create",data:{pageID:g.get("page-id")}});
o(k[b],b,i)}).always(function(){f.removeClass("disabled")});return!1}}var j=0,p=1,f,k={},i=q()&&"READ_ONLY"!==g.get("access-mode"),s=e.Deferred(),m={getLikesCache:function(){return s.promise()},showLikeUsers:function(a){a&&a.preventDefault();var a=e(this),b=a.data("content-id");a.blur();f&&(f.remove(),f=void 0);f=new d.Dialog(400,365,"likes-dialog");f.addHeader("People who like this");f.addPanel("Panel 1","<div class='spinner-container'></div>");f.addCancel("Close",
function(a){a.remove();f=void 0});f.getCurrentPanel().setPadding(0);f.show();e.ajax({type:"GET",url:n(b),data:{expand:"user",max:50},dataType:"json"}).done(function(a){if(f.popup.element.is(":visible")){a.showFollowActions=q();f.getCurrentPanel().html(h.Templates.Likes.likesDialog(a));e("#likes-dialog").find(".likes-dialog-follow-button").click(function(){var a=e(this);e.ajax({type:"PUT",url:d.contextPath()+"/rest/likes/1.0/user/"+g.get("remote-user")+"/following?username="+a.data("username"),contentType:"application/json",
dataType:"json"}).done(function(){a.replaceWith("Following")})})}})},appendAction:function(a){var a=a.find(".comment-actions-primary"),b=a.find("li[class~='comment-date']"),c=e(h.Templates.Likes.commentLikeSection({showLikeButton:i}));0===b.length?a.find("li:last-child").after(c):b.before(c)},reload:function(a){e.each(a,function(a,b){var d=e("#comment-"+a).find(".like-summary");o(b,a,d);k[a]=b});var b=e("#page-comments");!i&&b.find(".like-summary:empty").each(function(){e(this).closest(".comment-action-like").hide()});
b.find(".comment").each(function(){m.updateComment(e(this),a)});s.resolve(k)},updateComment:function(a,b){var c=a.attr("id");if(!c)return!0;var f=(/^comment-(\d+)$/.exec(c)||[])[1];if(!f)throw Error('Expecting ID attribute of comment to be in format "comment-XXX", found: '+c);c=b[f]&&g.get("remote-user")&&0<e.grep(b[f].likes,function(a){return a.user.name===g.get("remote-user")}).length;a.find(".like-button").click(c?l(p,f,"comment"):l(j,f,"comment")).find(".like-button-text").html(c?"Unlike":
"Like")},init:function(){var a=e(h.Templates.Likes.likeSection({showLikeButton:i}));g.get("page-id")&&e.ajax({type:"GET",url:n(g.get("page-id")),data:{commentLikes:!0},dataType:"json"}).done(function(b){if(g.get("remote-user")){var c=0<e.grep(b.likes,function(a){return a.user.name===g.get("remote-user")}).length,f=b.content_type;a.find(".like-button").click(c?l(p,g.get("page-id"),f):l(j,g.get("page-id"),f)).find(".like-button-text").html(c?"Unlike":"Like")}c=
a.find(".like-summary");o(b,b.content_id,c);""===c.html()&&!i&&a.hide();e("<div id='likes-and-labels-container' />").insertBefore("#labels-section").append(a).append(e("#labels-section"));k[b.content_id]=b;m.reload(b.commentLikes)});m.appendAction(e("#page-comments"))}};return m});
...
define("confluence-quick-edit/quick-edit","ajs confluence/legacy confluence/templates confluence/meta jquery window document confluence-quick-edit/util wrm".split(" "),function(b,e,q,i,c,r,s,t,u){function v(){var a=new c.Deferred;b.Confluence.EditorLoader.load(function(){setTimeout(function(){a.resolve()},0)},function(){a.reject()});return a}var o={enableShortcut:function(){c("#editPageLink").addClass("full-load")},disableShortcut:function(){c("#editPageLink").removeClass("full-load")}},g=[],n={loadingContentTimeout:4E3,
register:function(a){g.push(a)},disableHandlers:function(){c.each(g,function(a,b){return b.disable()})},enableHandlers:function(){c.each(g,function(a,b){return b.enable()})},SaveBarBinder:{bind:function(a,b){a&&e.Editor.addSaveHandler(a);b&&e.Editor.addCancelHandler(b)}},activateEditor:function(a){function e(){function f(a){var b=new c.Deferred;u.require(a).done(function(a){b.resolve(a)}).fail(function(a){b.reject(a)});return b}var g,j=new c.Deferred;if(b.Rte&&b.Rte.getEditor()){b.debug("there is already an editor open");
...
a.additionalResources?f(a.additionalResources):c.Deferred().resolve(),p)).done(function(e,f){var d={$container:a.$container,content:f,$form:a.$form,replayBufferedKeys:g};a.preInitialise&&a.preInitialise(d);c(".quick-comment-prompt",d.$container).hide();c(".quick-comment-body",d.$container).addClass("comment-body");var k=b.Confluence.EditorLoader.getPreloadContainer();if(d.content&&d.content.title){var m=d.content.title;k.find("#content-title").val(m)}d.$form.append(k.children());k.show();c("#editor-precursor").hide();
c("#rte-savebar").find(".toolbar-split-left").hide();if(i.get("content-type")==="comment"){c("#pluggable-status").hide();var k=q.Editor.Page.cancelButton({contentType:i.get("content-type"),sharedDraftsEnabled:i.getBoolean("shared-drafts")}),m=c("#rte-button-cancel"),l=m.parent(".rte-toolbar-group-done");if(l.length){l.remove();c("#rte-button-discard").remove()}else m.remove();c("#rte-savebar").find(".toolbar-split-right").append(k)}var h=function(){d.editor=b.Rte.getEditor();d.$container.find(".quick-comment-loading-container").hide();
d.$form.show();d.$form.addClass("fadeIn");var c=d.editor,e=d.content?d.content.editorContent:"",f=d.replayBufferedKeys;if(e){b.debug("Initial Editor Content from quick edit: ",e);c.setContent(e);c.startContent=c.getContent({format:"raw",no_events:1});c.undoManager.clear()}f()&&c.undoManager.add();b.trigger("quickedit.success");b.trigger("quickedit.visible");b.trigger("add-bindings.keyboardshortcuts");b.trigger("active.dynamic.rte");a.postInitialise&&a.postInitialise(d);n.SaveBarBinder.bind(a.saveHandler,
a.cancelHandler);b.trigger("rte-quick-edit-ready");c=i.get("content-type");i.get("collaborative-content")&&(c==="page"||c==="blogpost")&&b.trigger("rte-collab-editor-loaded");b.unbind("rte-ready",h);j.resolve()};b.bind("rte-ready",h);b.bind("rte-destroyed",a.postDeactivate||function(){});b.Rte.BootstrapManager.initialise({plugins:a.plugins,toolbar:a.toolbar,excludePlugins:a.excludePlugins,isQuickEdit:true,onInitialise:a.onInitialise})}).fail(function(c,e){j.reject(c,e);b.logError("Error loading page quick edit. Falling back to normal edit URL...");
b.trigger("analytics",{name:"rte.quick-edit.activate-editor.failed"});if(a.fallbackUrl){b.log("This parameter is deprecated. To be removed in the next major version (5.8 or 6.0). Please use the promise returned to bind custom action if the editor fails to load instead.");r.location=a.fallbackUrl}});return j.promise()}if(a.closeAnyExistingEditor&&b.Rte&&b.Rte.getEditor()){var f=new c.Deferred;this.deactivateEditor().done(function(){e().done(function(){f.resolve()}).fail(function(a){f.reject(a)})}).fail(function(){b.debug("Could not deactivate current editor.");
f.reject("Could not deactivate current editor.")});return f.promise()}return e()},deactivateEditor:function(){require("tinymce").majorVersion>=4?require("tinymce").execCommand("mceRemoveEditor",true,"wysiwygTextarea"):require("tinymce").execCommand("mceRemoveControl",true,"wysiwygTextarea");e.Editor.UI.toggleSavebarBusy(false);var a=b.Confluence.EditorLoader.getPreloadContainer().empty();c(".editor-container").remove();c("#editor-precursor").remove();c("#anonymous-warning").remove();c(".quick-comment-prompt").show();
c(".bottom-comment-panels").show();c("#editor-notification-container").empty();c(".action-reply-comment").removeAttr("reply-disabled");o.enableShortcut();e.Editor.removeAllSaveHandlers();e.Editor.removeAllCancelHandlers();return b.Confluence.EditorLoader.getEditorPreloadMarkup().done(function(c){a.append(c);a.hide();(new b.Confluence.QuickEditCaptchaManager(a)).refreshCaptcha();b.trigger("rte-destroyed");b.unbind("rte-destroyed")})}};return n});
